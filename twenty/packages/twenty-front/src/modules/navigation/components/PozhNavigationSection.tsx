...
// `внешний` — пункт, ведущий в ДРУГУЮ программу, а не в экран службы документов.
// Такой пока один: доска задач. Она единственная в системе смотрит в интернет,
// живёт по своему адресу и намеренно запрещает вкладывать себя в чужие
// страницы, поэтому рамкой её не открыть — только ссылкой в новой вкладке.
//
// Поле необязательное: служба документов постарше этой правки отдаёт список без
// него, и меню должно работать с обеими. Отсутствие читается как «внутренний».
type Экран = { ключ: string; название: string; адрес?: string; внешний?: boolean };
...
const РазделМеню = ({ раздел }: { раздел: Раздел }) => {
  ...
  return (
    <NavigationDrawerSection>
      ...
      >
        {раздел.экраны.map((э) =>
          э.внешний === true && э.адрес !== undefined ? (
            // Другая программа — открываем в новой вкладке и обычной ссылкой, а
            // не переходом внутри CRM: перехода туда нет, это чужой адрес.
            //
            // `rel` обязателен вместе с `target`: без него открытая страница
            // получает доступ к окну CRM и может увести человека на подделку
            // входа. Доска своя, но правило держится на свойстве ссылки, а не
            // на доверии к тому, что за ней.
            <NavigationDrawerItem
              key={э.ключ}
              label={э.название}
              to={э.адрес}
              Icon={IconFileText}
              onClick={() => window.open(э.адрес, '_blank', 'noopener,noreferrer')}
            />
          ) : (
            <NavigationDrawerItem
              key={э.ключ}
