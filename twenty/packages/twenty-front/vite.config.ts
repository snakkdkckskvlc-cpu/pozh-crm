...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      // пожсервис: адрес службы документов в политике содержимого.
      //
      // Он был вписан в разметку как localhost. На сервере компании адрес
      // другой, и рамка с нашими экранами оказалась бы ПУСТОЙ — молча: браузер
      // не пускает чужой адрес, а в журналах при этом чисто.
      //
      // Значение берётся из настройки REACT_APP_POZH_DOCS_URL. Если её нет,
      // подставляется свой же адрес — на машине разработчика служба там и живёт.
      {
        name: 'pozh-docs-origin',
        transformIndexHtml(html: string) {
          const адрес = env.REACT_APP_POZH_DOCS_URL || 'http://localhost:8000';
          let источник = адрес;

          try {
            // В политику содержимого идёт только имя и порт: путь она не
            // понимает вовсе и молча отбрасывает всё правило целиком.
            источник = new URL(адрес).origin;
          } catch {
            источник = адрес;
          }

          return html.replace(/%POZH_DOCS_ORIGIN%/g, источник);
        },
      },
      react({
