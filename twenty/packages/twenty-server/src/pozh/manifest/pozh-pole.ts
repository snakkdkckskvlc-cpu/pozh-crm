...
type ОбщееПоля = {
  ...
  уникально?: boolean;
  обязательно?: boolean;
  // «Поле правит программа» — на экране оно видно, но руками не меняется.
  //
  // ОСТОРОЖНО, ЭТО НЕ ЗАПРЕТ. Признак работает в браузере; на сервере проверки
  // нет, и запрос, посланный мимо экрана, поле изменит. Настоящий запрет
  // ставится правом роли на поле. Здесь — защита от случайной правки.
  //
  // Признак применяют ВСЕ помощники ниже. Так было не всегда: сначала его умел
  // только `текст`, а остальные принимали его и молча выбрасывали — то есть
  // поле, помеченное в описании как «правит программа», на экране правилось
  // руками, и заметить это можно было только в браузере. Поймано 20.08.2026
  // при заведении совещаний. Заводя новый помощник, не забудьте строку с
  // `isUIEditable`.
