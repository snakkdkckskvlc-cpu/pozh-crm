...
const главное = async () => {
  ...
  //
  // Раньше здесь печаталась простыня из первых двух тысяч знаков — и она
  // обрезалась на середине, показав три удаления из шести. Отчёт, которому
  // нельзя верить, хуже отсутствующего: по нему я чуть не выложил вслепую.
  // Теперь печатается пересчёт по видам, а он не обрезается.
  const опасные = (действия as { type?: string; metadataName?: string }[]).filter((д) =>
    (д.type ?? '').toLowerCase().includes('delete'),
  );

  if (опасные.length > 0) {
    const поВидам = new Map<string, number>();

    for (const д of опасные) {
      const вид = д.metadataName ?? 'вид не указан';

      поВидам.set(вид, (поВидам.get(вид) ?? 0) + 1);
    }

    console.log(`\n!!! УДАЛЕНИЙ: ${опасные.length}`);

    for (const [вид, сколько] of [...поВидам].sort()) {
      // objectMetadata здесь означает «таблица вместе с данными» — это то,
      // ради чего просмотр вообще существует.
      const тревога = вид === 'objectMetadata' ? '  <-- ТАБЛИЦА С ДАННЫМИ' : '';

      console.log(`  ${вид}: ${сколько}${тревога}`);
    }
  }
};
