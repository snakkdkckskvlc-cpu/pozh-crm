...
export const выбор = ({
  ...
  label: подпись,
  ...(пояснение === undefined ? {} : { description: пояснение }),
  options: варианты.map((в, i) => ({ ...в, position: i })),
  ...(поумолчанию === undefined
    ? { isNullable: true }
    : { defaultValue: `'${поумолчанию}'`, isNullable: false }),
});
...
export const источник = (объект: string): ObjectFieldManifest[] => [
  текст({
    ...
    пояснение: 'Откуда приехала запись. Заполняет перенос, человеку менять не надо.',
    обязательно: true,
    правитЧеловек: false,
  }),
  текст({
